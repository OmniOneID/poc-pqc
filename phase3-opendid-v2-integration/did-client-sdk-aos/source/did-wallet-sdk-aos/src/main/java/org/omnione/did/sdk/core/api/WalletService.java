...
import org.omnione.did.sdk.datamodel.common.enums.AlgorithmType; // PQC:
...
import org.omnione.did.sdk.core.keymanager.supportalgorithm.MlDsa44SignatureCompat;
...
public class WalletService implements WalletServiceInterface {
        ...
        for(String keyId : keyIds) {
            ...
            switch (keyId) {
                ...
                case Constants.KEY_ID_BIO:
                    proof = createProof(did, ProofPurpose.PROOF_PURPOSE.assertionMethod, keyId, type); // PQC:
                    if(isDIDAuth)
                        proof = createProof(did, ProofPurpose.PROOF_PURPOSE.authentication, keyId, type); // PQC:
                    break;
                case Constants.KEY_ID_AUTH:
                    proof = createProof(did, ProofPurpose.PROOF_PURPOSE.authentication, keyId, type); // PQC:
                    break;
                case Constants.KEY_ID_KEY_AGREE:
                    proof = createProof(did, ProofPurpose.PROOF_PURPOSE.keyAgreement, keyId, type); // PQC:
                    break;
                default:
                    throw new WalletException(WalletErrorCode.ERR_CODE_WALLET_CREATE_PROOF_FAIL);
            }
            document.setProof(proof);
            document.setProofs(null); // PQC: 서명 전 proofs 초기화 (서명 대상 JSON에 포함되면 서버 검증 불일치)
            byte[] signature = null;
            ...
            if(signature == null)
                throw new WalletException(WalletErrorCode.ERR_CODE_WALLET_CREATE_PROOF_FAIL);
            // PQC: ML-DSA-44 signatures are large; use BASE_64 encoding
    ...
    // PQC: look up key's algorithm to set the correct proof type
    private Proof createProof(String did, ProofPurpose.PROOF_PURPOSE proofPurpose, String keyId, int type) {
        Proof proof = new Proof();
        ...
        try {
            algo = walletCore.getKeyAlgorithmType(keyId, type);
        } catch (Exception ignored) {}
        proof.setType(ProofType.PROOF_TYPE.convertFrom(algo));
...
//        List<VerifiableCredential> vcList = walletCore.getAllCredentials();
    ...
    private boolean verifyProof(String strVc, boolean isCertVc, String apiGateWayUrl) throws WalletCoreException, UtilityException, WalletException, ExecutionException, InterruptedException {
        ...
        String encodePubKey = "";
        DIDKeyType.DID_KEY_TYPE keyType = null;
        ...
        walletLogger.d("verifyProof(walletService) did=" + did
        ...
        walletLogger.d("verifyProof JSON length=" + vcJsonForVerify.length()
                + " / sha256=" + sha256Hex(vcJsonForVerify)
                + " / json=" + vcJsonForVerify);
        if (algorithmType == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
            walletLogger.d("verifyProof ML-DSA diagnose before verify: "
                    + MlDsa44SignatureCompat.diagnoseVerify(
                    publicKey,
                    vcJsonForVerify.getBytes(StandardCharsets.UTF_8),
                    signature));
        }
        ...
        if (!result && algorithmType == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
            walletLogger.d("verifyProof ML-DSA diagnose after verify(false): "
                    + MlDsa44SignatureCompat.diagnoseVerify(
                    publicKey,
                    vcJsonForVerify.getBytes(StandardCharsets.UTF_8),
                    signature));
