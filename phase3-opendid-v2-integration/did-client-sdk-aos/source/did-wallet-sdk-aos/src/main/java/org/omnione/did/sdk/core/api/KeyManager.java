...
import org.omnione.did.sdk.core.keymanager.supportalgorithm.MlDsa44Manager; // PQC:
...
class KeyManager<E extends BaseObject>{
    ...
    public void authenticatePin(String id, byte[] pin) throws WalletCoreException, UtilityException {
        ...
        switch (signkeyInfo.getAccessMethod()) {
            case WALLET_PIN:
                ...
                // PQC: check key pair match based on algorithm
                if (signkeyInfo.getAlgorithm() == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
                    new MlDsa44Manager().checkKeyPairMatch(PINDecoded, publicKey);
                } else {
                    new Secp256R1Manager().checkKeyPairMatch(PINDecoded, publicKey);
                }
    ...
    private void generateKeyForWallet(WalletKeyGenRequest keyGenRequest) throws WalletCoreException, UtilityException {
        ...
        KeyGenerationInfo genKeyInfo;
        // PQC: dispatch to the correct algorithm manager
        SignableInterface keyAlgorithmForGen = getKeyAlgorithm(keyGenRequest.getAlgorithmType());
        genKeyInfo = keyAlgorithmForGen.generateKey();
    ...
    public void changePin(String id, byte[] oldPin, byte[] newPin) throws WalletCoreException, UtilityException {
        ...
        // PQC: verify key pair based on algorithm
        if (oldKeyInfo.getAlgorithm() == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
            new MlDsa44Manager().checkKeyPairMatch(priKey, pubKey);
        } else if(oldKeyInfo.getAlgorithm().getValue().equals(AlgorithmType.ALGORITHM_TYPE.SECP256R1.getValue())){
    ...
    public byte[] sign(String id, byte[] pin, byte[] digest) throws WalletCoreException, UtilityException {
        ...
        switch (signkeyInfo.getAccessMethod()) {
            case WALLET_NONE:
                ...
                if (signkeyInfo.getAlgorithm() == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) { // PQC:
                    signature = new MlDsa44Manager().sign(decPrivateKey, digest);
                } else if(signkeyInfo.getAlgorithm().getValue().equals("Secp256r1")){
                    signature = new Secp256R1Manager().sign(decPrivateKey, digest);
                }
            ...
            case WALLET_PIN:
                ...
                // PQC: check key pair and sign based on algorithm
                if (signkeyInfo.getAlgorithm() == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
                    new MlDsa44Manager().checkKeyPairMatch(PINDecoded, publicKey);
                    signature = new MlDsa44Manager().sign(PINDecoded, digest);
                } else {
    ...
    public boolean verify(AlgorithmType.ALGORITHM_TYPE algorithmType, byte[] publicKey, byte[] digest, byte[] signature) throws WalletCoreException {
        ...
        switch (algorithmType) {
            case SECP256R1:
                ...
                break;
            case ML_DSA_44: // PQC:
                result = new MlDsa44Manager().verify(publicKey, digest, signature);
                break;
            default:
                throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_UNSUPPORTED_ALGORITHM);
        }
    ...
    private <T extends SignableInterface> T getKeyAlgorithm(AlgorithmType.ALGORITHM_TYPE algorithmType) throws WalletCoreException{
        switch (algorithmType){
            ...
            case ML_DSA_44: // PQC:
                return (T) new MlDsa44Manager();
            default:
                throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_UNSUPPORTED_ALGORITHM, algorithmType.getValue());
        }
    }

    // PQC: Returns the algorithm type of the key with the given id
    public AlgorithmType.ALGORITHM_TYPE getKeyAlgorithmType(String id) throws WalletCoreException, UtilityException {
        List<String> identifiers = new ArrayList<>();
        identifiers.add(id);
        List<KeyInfo> metas = storageManager.getMetas(identifiers);
        if (metas == null || metas.isEmpty()) return AlgorithmType.ALGORITHM_TYPE.SECP256R1;
        return metas.get(0).getAlgorithm();
    }
}
