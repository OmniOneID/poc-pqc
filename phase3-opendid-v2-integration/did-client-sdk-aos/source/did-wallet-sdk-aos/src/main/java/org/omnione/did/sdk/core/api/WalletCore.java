...
//        keyManager.generateKey(keyGenInfo);
    ...
    public DIDDocument createDeviceDIDDoc() throws WalletCoreException, UtilityException {
        if(!deviceKeyManager.isAnyKeySaved()) {
            // PQC: use configured signing algorithm; keyagree stays SECP256R1 for ECDH
            AlgorithmType.ALGORITHM_TYPE sigAlgo = AlgorithmType.ALGORITHM_TYPE.fromValue(Config.KEY_ALGORITHM);
            if (sigAlgo == null) sigAlgo = AlgorithmType.ALGORITHM_TYPE.SECP256R1;
            ...
            keyGenInfo = new WalletKeyGenRequest(
                    Constants.KEY_ID_KEY_AGREE,
                    AlgorithmType.ALGORITHM_TYPE.SECP256R1, // PQC: keyagree stays EC for ECDH
                    StorageOption.STORAGE_OPTION.WALLET,
                    keyGenWalletMethodType
            );
            deviceKeyManager.generateKey(keyGenInfo);
    ...
    public DIDDocument createHolderDIDDoc() throws WalletCoreException, UtilityException, WalletException {
        ...
        if(!keyManager.isKeySaved(Constants.KEY_ID_KEY_AGREE)) {
            // PQC: keyagree always stays SECP256R1 for ECDH key exchange
            KeyGenWalletMethodType keyGenWalletMethodType = new KeyGenWalletMethodType();
            WalletKeyGenRequest keyGenInfo = new WalletKeyGenRequest(
                    Constants.KEY_ID_KEY_AGREE,
                    AlgorithmType.ALGORITHM_TYPE.SECP256R1,
    ...
    public byte[] sign(String id, byte[] pin, byte[] digest, int type) throws WalletCoreException, UtilityException, WalletException {
        ...
        // PQC: ML-DSA-44 signs raw data; EC algorithms need SHA-256 pre-hash
        AlgorithmType.ALGORITHM_TYPE algo;
        try {
            if (type == Constants.DID_DOC_TYPE_DEVICE)
                algo = deviceKeyManager.getKeyAlgorithmType(id);
            else
                algo = keyManager.getKeyAlgorithmType(id);
        } catch (Exception e) {
            algo = AlgorithmType.ALGORITHM_TYPE.SECP256R1;
        }

        byte[] dataToSign;
        if (algo == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
            dataToSign = digest; // raw data — ML-DSA-44 handles hashing internally
        } else {
            dataToSign = DigestUtils.getDigest(digest, DigestEnum.DIGEST_ENUM.SHA_256);
        }

        byte[] signValue = null;
        if(type == Constants.DID_DOC_TYPE_DEVICE)
            signValue = deviceKeyManager.sign(id, pin, dataToSign);
        else if(type == Constants.DID_DOC_TYPE_HOLDER)
            signValue = keyManager.sign(id, pin, dataToSign);
        return signValue;
    }
    ...
    public boolean verify(byte[] publicKey, byte[] digest, byte[] signature) throws WalletCoreException, UtilityException, WalletException {
        return verify(AlgorithmType.ALGORITHM_TYPE.SECP256R1, publicKey, digest, signature);
    }

    public boolean verify(AlgorithmType.ALGORITHM_TYPE algorithmType, byte[] publicKey, byte[] digest, byte[] signature) throws WalletCoreException, UtilityException, WalletException {
        if(WalletApi.isLock)
            throw new WalletException(WalletErrorCode.ERR_CODE_WALLET_LOCKED_WALLET);
        ...
        if (algorithmType == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
            ...
            dataToVerify = DigestUtils.getDigest(digest, DigestEnum.DIGEST_ENUM.SHA_256);
        }

        boolean result = keyManager.verify(algorithmType, publicKey, dataToVerify, signature);
        return result;
    }
    ...
    // PQC: returns the algorithm type for a given key id; used by WalletService.createProof()
    AlgorithmType.ALGORITHM_TYPE getKeyAlgorithmType(String id, int type) throws WalletCoreException, UtilityException {
        try {
            if (type == Constants.DID_DOC_TYPE_DEVICE)
                return deviceKeyManager.getKeyAlgorithmType(id);
            else
                return keyManager.getKeyAlgorithmType(id);
        } catch (Exception e) {
