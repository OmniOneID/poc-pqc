...
//            }
    ...
    private static byte[] copyPublicKey(ECPublicKey ecPublicKey) throws WalletCoreException{
        ...
        compressedPublicKey[0] = point.getAffineY().testBit(0) ? (byte) 0x03 : (byte) 0x02;
        byte[] xBytes = toFixedLength(point.getAffineX(), 32);
        System.arraycopy(xBytes, 0, compressedPublicKey, 1, xBytes.length);
        Arrays.fill(xBytes, (byte) 0x00);
        return compressedPublicKey;
    }

    private static byte[] toFixedLength(BigInteger value, int size) throws WalletCoreException {
