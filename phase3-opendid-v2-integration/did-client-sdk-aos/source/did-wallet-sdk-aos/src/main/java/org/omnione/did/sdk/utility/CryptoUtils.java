...
public class CryptoUtils {
    ...
    // PQC: ML-KEM-768 키페어 컨테이너 (X.509 공개키 + PKCS#8 개인키)
    public static class MlKemKeyPair {
        private final byte[] publicKey;
        private final byte[] privateKey;

        public MlKemKeyPair(byte[] publicKey, byte[] privateKey) {
    ...
    // PQC: BouncyCastle 1.79+ provider (SpongyCastle과 별개로 ML-KEM 지원)
    private static final Provider BC_PROVIDER = new org.bouncycastle.jce.provider.BouncyCastleProvider();

    /**
     * ML-KEM-768 임시 키페어 생성.
     * 공개키(X.509)는 서버로 전송하고, 개인키(PKCS#8)는 decapsulate에 사용한다.
     */
    public static MlKemKeyPair generateMLKEMKeyPair() throws UtilityException {
        try {
            KeyPairGenerator kpg = KeyPairGenerator.getInstance("ML-KEM-768", BC_PROVIDER);
     ...
     * 서버 응답의 ciphertext를 자신의 임시 개인키로 풀어 sharedSecret(32B)을 도출한다.
     *
     * @param privateKeyBytes PKCS#8 인코딩 개인키
     * @param ciphertext      서버 AccEcdh.ciphertext (multibase 디코딩 후 전달)
     * @return sharedSecret (32B)
     */
    public static byte[] mlKemDecapsulate(byte[] privateKeyBytes, byte[] ciphertext) throws UtilityException {
        try {
            KeyFactory kf = KeyFactory.getInstance("ML-KEM-768", BC_PROVIDER);
            PrivateKey privateKey = kf.generatePrivate(new PKCS8EncodedKeySpec(privateKeyBytes));
            org.bouncycastle.jcajce.spec.KEMExtractSpec spec =
                    new org.bouncycastle.jcajce.spec.KEMExtractSpec(privateKey, ciphertext, "AES");
            javax.crypto.KeyGenerator kg = javax.crypto.KeyGenerator.getInstance("ML-KEM-768", BC_PROVIDER);
            kg.init(spec);
