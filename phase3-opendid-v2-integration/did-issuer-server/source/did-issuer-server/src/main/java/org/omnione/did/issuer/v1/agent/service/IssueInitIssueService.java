...
public class IssueInitIssueService extends IssueServiceBase {
    ...
    @Value("${issuer.test.auto-create-user:false}")
    private boolean autoCreateTestUser;

    private static final String DUMMY_DATA_TEMPLATE =
            "{\"did\":\"did:omn:test\",\"firstname\":\"TE\",\"lastname\":\"ST\"," +
            "\"userName\":\"TEST\",\"birthdate\":\"2024-01-01\",\"address\":\"TEST\"," +
            "\"licenseNum\":\"123123-123123\",\"issueDate\":\"2024-01-01\",\"pii\":\"%s\"}";
    ...
    public IssueInitIssueService(VcProfileQueryService vcProfileQueryService, VcOfferQueryService vcOfferQueryService,
                                 ...
                                 ZkpSchemaQueryService zkpSchemaQueryService, WalletProperty walletProperty) { // PQC:
        super(vcProfileQueryService, vcOfferQueryService, transactionService, e2EQueryService, vcQueryService
                ...
                zkpSchemaQueryService, walletProperty); // PQC:
        ...
        return userQueryService.findByPii(holder.getPii())
                .orElseGet(() -> {
                    if (!autoCreateTestUser) {
                        throw new OpenDidException(ErrorCode.HOLDER_NOT_FOUND);
                    }
                    log.info("auto-create-user: creating dummy user for pii={}", holder.getPii());
                    User dummy = User.builder()
                            .pii(holder.getPii())
                            .vcSchemaId(1L)
                            .data(String.format(DUMMY_DATA_TEMPLATE, holder.getPii()))
                            .build();
                    return userQueryService.save(dummy);
                });
    }
    ...
    protected User findUserByHolderAndVcSchemaId(Holder holder, Long vcSchemaId) {
        return userQueryService.findByPiiAndVcSchemaId(holder.getPii(), vcSchemaId)
                .orElseGet(() -> {
                    ...
                    log.info("auto-create-user: creating dummy user for pii={}, vcSchemaId={}", holder.getPii(), vcSchemaId);
                    User dummy = User.builder()
                            .pii(holder.getPii())
