...
public abstract class IssueServiceBase implements IssueService {
    ...
    private String encryptVerifiableCredential(VerifiableCredential verifiableCredential, Credential credential, byte[] sharedSecretKey, byte[] iv, E2E e2e) {
        ...
        log.debug("\t--> CredentialInfo JSON length={} / sha256={} / json={}",
                credentialJson.length(), sha256Hex(credentialJson), credentialJson);
    ...
    private VerifiableCredential issueVerifiableCredential(VcManager vcManager, String holderDid, String data
        ...
        try {
            ...
            logBcDirectVerify(didDocument, signatureParams);

            verifiableCredential = vcManager.addProof(verifiableCredential, signatureParams);
            String issuedVcJson = verifiableCredential.toJson();
            log.debug("\t--> Issued VC JSON length={} / sha256={} / json={}",
                    issuedVcJson.length(), sha256Hex(issuedVcJson), issuedVcJson);
    ...
    private void signVc(List<SignatureVcParams> signatureParams) {
        for (SignatureParams signatureParam : signatureParams) {
            ...
            log.debug("[SIGN] originData length={} / sha256={} / json={}",
                    originData.length(), sha256Hex(originData), originData);
    ...
    private void logBcDirectVerify(DidDocument didDocument, List<SignatureVcParams> signatureParams) {
        String assertPublicKeyMultibase = null;
        for (VerificationMethod verificationMethod : didDocument.getVerificationMethod()) {
            if ("assert".equals(verificationMethod.getId())) {
                assertPublicKeyMultibase = verificationMethod.getPublicKeyMultibase();
                break;
            }
        }

        if (assertPublicKeyMultibase == null) {
            log.debug("[SIGN][BC-VERIFY] assert public key not found in did document");
            return;
        }

        byte[] publicKeyBytes = BaseMultibaseUtil.decode(assertPublicKeyMultibase);
        for (SignatureVcParams signatureParam : signatureParams) {
            String originData = signatureParam.getOriginData();
            String signatureValue = signatureParam.getSignatureValue();
            byte[] signatureBytes = BaseMultibaseUtil.decode(signatureValue);
            try {
                SignatureUtils.verifyMlDsa44Signature(
                        publicKeyBytes,
                        originData.getBytes(StandardCharsets.UTF_8),
                        signatureBytes);
                log.debug("[SIGN][BC-VERIFY] keyId={} / publicKeyLength={} / originSha256={} / signatureLength={} / result=true",
                        signatureParam.getKeyId(),
                        publicKeyBytes.length,
                        sha256Hex(originData),
                        signatureBytes.length);
            } catch (Exception e) {
                log.debug("[SIGN][BC-VERIFY] keyId={} / publicKeyLength={} / originSha256={} / signatureLength={} / result=false / error={}({})",
                        signatureParam.getKeyId(),
                        publicKeyBytes.length,
                        sha256Hex(originData),
                        signatureBytes.length,
