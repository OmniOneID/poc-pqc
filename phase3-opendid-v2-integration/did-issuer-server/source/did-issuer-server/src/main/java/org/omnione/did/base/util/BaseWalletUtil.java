...
public class BaseWalletUtil {
    ...
    public static byte[] generateCompactSignature(WalletManagerInterface walletManager, String keyId, byte[] plainText) {
        try {
            ...
            boolean isMlDsa44Key = keyElement != null
                    && CryptoKeyPairInfo.KeyAlgorithmType.ML_DSA_44.equals(keyElement.getAlgorithm());

            // keyagree is always EC-based for ECDH. ML-DSA signing keys must sign raw payloads.
            if (isMlDsa44Key && !"keyagree".equals(keyId)) {
    ...
    // PQC: keyId별 알고리즘 지정 초기화
    public static WalletManagerInterface initializeWalletWithKeys(String walletFilePath, String password,
            Map<String, CryptoKeyPairInfo.KeyAlgorithmType> keyAlgorithms) {
        createFileWalletSafe(walletFilePath, password);
        WalletManagerInterface walletManager = connectFileWallet(walletFilePath, password);
