...
public class ValidationUtil {
    ...
    public static void verifySign(RequestProof request, DidDocument didDocument) {
        ...
        // PQC: proof type에 따라 ML-DSA-44 또는 EC 검증 분기
        if (ProofType.ML_DSA_44_SIGNATURE_2024.getRawValue().equals(proof.getType())) {
            try {
                byte[] publicKeyBytes = BaseMultibaseUtil.decode(publicKeyByKeyId.getPublicKeyMultibase());
                byte[] signatureBytes = BaseMultibaseUtil.decode(proof.getProofValue());
                SignatureUtils.verifyMlDsa44Signature(publicKeyBytes, source.getBytes(StandardCharsets.UTF_8), signatureBytes);
