...
public class VcIssuance extends BaseOperation {
    ...
    private CompletableFuture<ProtocolData> fetchVcPlanList() {
        ...
        String result = sendTimed(protocolData, Config.TAS.VC_PLAN_LIST, "GET", "");
        ...
        String targetPlanId = (requestDataMap != null) ? requestDataMap.get("vcPlanId") : null;
        int selectedIndex = 0;
        if (targetPlanId != null) {
            for (int i = 0; i < vcPlanList.getItems().size(); i++) {
                if (targetPlanId.equals(vcPlanList.getItems().get(i).getVcPlanId())) {
                    selectedIndex = i;
                    break;
                }
            }
        ...
        protocolData.setIssuer(vcPlanList.getItems().get(selectedIndex).getAllowedIssuers());
        protocolData.setVcPlanId(vcPlanList.getItems().get(selectedIndex).getVcPlanId());
        completableFuture.complete(protocolData);
        return completableFuture;
    }
    ...
    private CompletableFuture<ProtocolData> proposeIssueVc(ProtocolData protocolData) {
        ...
        String result = sendTimed(protocolData, Config.TAS.PROPOSE_ISSUE_VC, "POST", req.toJson());
    ...
    private CompletableFuture<ProtocolData> requestIssueProfile(ProtocolData protocolData) {
        ...
        String result = sendTimed(protocolData, Config.TAS.REQUEST_ISSUE_PROFILE, "POST", req.toJson());
    ...
    private CompletableFuture<ProtocolData> requestIssueVc(ProtocolData protocolData) {
        ...
        try {
            ...
            protocolData.addServerRoundTripNs(System.nanoTime() - serverStartNs);
    ...
    public void preExecute() {
        ...
        .thenComposeAsync(this::requestECDH)
