...
public class TokenUtil {
    ...
    // PQC: ML-KEM-768 세션 토큰 생성 오버로드
    public static String createServerToken(String std, String ecdh, byte[] clientNonce, byte[] kemPrivateKey) throws UtilityException {
        String iv = MessageUtil.deserialize(std, P210ResponseVo.class).getIv();
        String encStd = MessageUtil.deserialize(std, P210ResponseVo.class).getEncStd();
        AccEcdh accEcdh = MessageUtil.deserialize(ecdh, P210ResponseVo.class).getAccEcdh();
        CipherInfo info = new CipherInfo(
                CipherInfo.ENCRYPTION_TYPE.fromValue(accEcdh.getCipher().getValue().split("-")[0]),
                CipherInfo.ENCRYPTION_MODE.fromValue(accEcdh.getCipher().getValue().split("-")[2]),
                CipherInfo.SYMMETRIC_KEY_SIZE.fromValue(accEcdh.getCipher().getValue().split("-")[1]),
                CipherInfo.SYMMETRIC_PADDING_TYPE.fromKey(accEcdh.getPadding().getValue()));

        byte[] sessKey = createKEMSecret(clientNonce, accEcdh, kemPrivateKey);
        byte[] plain = CryptoUtils.decrypt(MultibaseUtils.decode(encStd), info, sessKey, MultibaseUtils.decode(iv));
        String serverToken = MultibaseUtils.encode(MultibaseType.MULTIBASE_TYPE.BASE_58_BTC, DigestUtils.getDigest(new String(plain).getBytes(), DigestEnum.DIGEST_ENUM.SHA_256));
        CaLog.d("serverToken (ML-KEM) by CA : " + serverToken);
        return serverToken;
    }

    // PQC: ML-KEM decapsulate → sharedSecret → sessionKey 도출
    public static byte[] createKEMSecret(byte[] clientNonce, AccEcdh accEcdh, byte[] kemPrivateKey) throws UtilityException {
        ...
        return mergeSharedSecretAndNonce(sharedSecret, mergedNonce, accEcdh.getCipher());
    }
