...
public class Preference {
    ...
    public static void setPiiForDemo(Context context, String pii) {
        SharedPreferences prefs = getPreference(context);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putString("pii_for_demo", pii);
        editor.apply();
    }
    public static String getPiiForDemo(Context context) {
        SharedPreferences prefs = getPreference(context);
        return prefs.getString("pii_for_demo", "");
    }
    ...
    // Iteration Reset: 사용자/지갑 관련 상태만 제거, caAppId는 유지
    public static void resetForIteration(Context context) {
        SharedPreferences prefs = getPreference(context);
        SharedPreferences.Editor editor = prefs.edit();
        editor.remove(Constants.PREFERENCE_INIT);
        editor.remove(Constants.PREFERENCE_USER_NAME_FOR_DEMO);
        editor.remove(Constants.PREFERENCE_USER_ID_FOR_KYC);
        editor.remove("pii_for_demo");
        editor.remove(Constants.PREFERENCE_DID);
        editor.remove(Constants.PREFERENCE_DEVICE_DID);
        editor.remove(Constants.PREFERENCE_PROFILE);
        editor.remove(Constants.PREFERENCE_DEFAULT_AUTH);
        editor.remove(Constants.PREFERENCE_PUSH_TOKEN);
        editor.apply();
    }
