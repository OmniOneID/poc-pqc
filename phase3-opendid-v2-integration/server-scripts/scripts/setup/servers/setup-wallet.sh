...
# =============================================================
# setup-wallet.sh - OpenDID PoC Wallet Server Setup Automation
# =============================================================
# Prerequisites:
...
#
# Usage:
#   ./setup-wallet.sh [options]
#
# Options:
#   --ip <ip>                 Server IP address
#   --wallet-url <url>        Wallet server base URL (default: derived from --ip)
#   --tas-url <url>           TA server base URL (default: derived from --ip)
#   --admin-id <id>           Wallet admin login ID (default: admin@opendid.omnione.net)
#   --ta-admin-id <id>        TA admin login ID (default: admin@opendid.omnione.net)
#   --wallet-name <name>      Wallet service name (default: "OpenDID Wallet")
...
WALLET_URL=""
TAS_URL=""
WALLET_ADMIN_ID="admin@opendid.omnione.net"
TA_ADMIN_ID="admin@opendid.omnione.net"
WALLET_NAME="OpenDID Wallet"
WORK_DIR="$PROJECT_ROOT/logs/wallet-setup"
LOCK_DIR=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --admin-id)      WALLET_ADMIN_ID="$2"; shift 2 ;;
...
# --- Resolve IP / URLs ---
if [ -z "$_LOCAL_IP" ] && [ -z "$WALLET_URL" ]; then
    read -rp "Server IP: " _LOCAL_IP
fi

if [ -z "$WALLET_URL" ]; then
    WALLET_URL="http://${_LOCAL_IP}:8095/wallet"
fi
...
WALLET_SERVER_URL="$WALLET_URL"

ADMIN_BASE="$WALLET_URL/admin/v1"
TAS_ADMIN_BASE="$TAS_URL/admin/v1"

WALLET_COOKIE="$WORK_DIR/wallet-cookies.txt"
...
echo ""
echo "============================================================="
echo "  Wallet Server Setup"
echo "============================================================="
echo "  WALLET_URL : $WALLET_URL"
echo "  TAS_URL    : $TAS_URL"
echo "  ADMIN_ID   : $WALLET_ADMIN_ID"
echo "  WORK_DIR   : $WORK_DIR"
echo "============================================================="
echo ""

# =============================================================
# Step 1: Wallet admin 로그인
# =============================================================

post_json "Step 1: Wallet admin login" \
    "$ADMIN_BASE" "/login" \
    "{\"loginId\": \"${WALLET_ADMIN_ID}\", \"loginPassword\": \"${PASSWORD_HASH}\"}" \
    "$WALLET_COOKIE"
...
if [[ "$REQUIRE_RESET" == "true" ]]; then
    post_json "Step 2: Reset password" \
        ...
        "{\"loginId\": \"${WALLET_ADMIN_ID}\", \"oldPassword\": \"${PASSWORD_HASH}\", \"newPassword\": \"${PASSWORD_HASH}\"}" \
        "$WALLET_COOKIE"
...
# =============================================================
# Step 4: Wallet 서비스 정보 등록
# =============================================================

post_json "Step 4: Register wallet service info" \
    "$ADMIN_BASE" "/wallet-service/register-wallet-service-info" \
    "{\"name\": \"${WALLET_NAME}\", \"serverUrl\": \"${WALLET_SERVER_URL}\"}" \
    "$WALLET_COOKIE"

save_artifact "04-register-wallet-service-info.json" "$RESPONSE"
...
post_json "Step 5: Generate DID auto" \
    "$ADMIN_BASE" "/wallet-service/generate-did-auto" \
    "" \
    "$WALLET_COOKIE"
...
post_json "Step 6: Register DID to TA" \
    "$ADMIN_BASE" "/wallet-service/register-did" \
    "$REGISTER_DID_BODY" \
    "$WALLET_COOKIE"
...
ENTITY_ID=$(echo "$RESPONSE" | jq -r '.content[] | select(.status == "DID_DOCUMENT_REQUIRED" and .role == "WALLET_PROVIDER") | .id' | head -n 1)
if [ -z "$ENTITY_ID" ] || [ "$ENTITY_ID" = "null" ]; then
    LAST_RESPONSE="$RESPONSE"
    fail "Wallet Entity ID not found in TA entity list (no DID_DOCUMENT_REQUIRED + WALLET_PROVIDER entity)"
fi

echo "   Entity ID: $ENTITY_ID"

# =============================================================
# Step 9: TA에서 Wallet DID 승인
# =============================================================
...
get_json "Step 10: Check request status" \
    "$ADMIN_BASE" "/wallet-service/request-status" \
    "$WALLET_COOKIE"
...
post_json "Step 11: Enroll entity" \
    "$ADMIN_BASE" "/wallet-service/request-enroll-entity" \
    "" \
    "$WALLET_COOKIE"
...
get_json "Step 12: Verify final state" \
    "$ADMIN_BASE" "/wallet-service/info" \
    "$WALLET_COOKIE"

save_artifact "12-wallet-info.json" "$RESPONSE"
