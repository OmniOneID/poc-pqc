...
public class TokenServiceImpl implements TokenService {
    ...
    private byte[] extractSignatureMessage(SignedWalletInfo signedWalletInfo) {
        try {
            ...
            return prepareSignatureMessage(jsonString, signedWalletInfo.getProof().getType());
        } catch (OpenDidException e) {
          throw e;
        } catch (Exception e) {
    ...
    private void verifySignature(String encodedPublicKey, String signature, byte[] signatureMassage, ProofType proofType) {
        try {
            signatureService.verifySignature(encodedPublicKey, signature, signatureMassage, proofType);
        } catch (OpenDidException e) {
            throw e;
        } catch (Exception e) {
    ...
    private byte[] extractSignatureMessage(AttestedAppInfo attestedAppInfo) {
        try {
            ...
            return prepareSignatureMessage(jsonString, attestedAppInfo.getProof().getType());
    ...
    private byte[] prepareSignatureMessage(String jsonString, ProofType proofType) throws OpenDidException {
