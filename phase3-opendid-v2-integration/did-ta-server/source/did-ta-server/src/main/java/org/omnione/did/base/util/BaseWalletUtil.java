...
public class BaseWalletUtil {
    ...
    public static void generateKeyPair(WalletManagerInterface walletManager, String keyId) {
        generateKeyPair(walletManager, keyId, CryptoKeyPairInfo.KeyAlgorithmType.SECP256r1);
    }

    // forten - 알고리즘 지정 키 생성
    public static void generateKeyPair(WalletManagerInterface walletManager, String keyId, CryptoKeyPairInfo.KeyAlgorithmType algorithmType) {
        try {
            walletManager.generateRandomKey(keyId, algorithmType);
        } catch (WalletException e) {
    ...
    /**
     * Generates a compact signature with algorithm-aware hashing.
     * ML-DSA-44 signs raw data directly; ECC signs SHA-256 hashed data.
     *
     * @param walletManager WalletManagerInterface instance for managing wallet operations.
     * @param keyId Key ID of the key to use for signing.
     * @param plainText Data to sign as a byte array.
     * @param algorithm Proof type algorithm string.
     * @return Compact signature as a byte array.
     * @throws OpenDidException if signature generation fails.
     */
    public static byte[] generateCompactSignature(WalletManagerInterface walletManager, String keyId, byte[] plainText, String algorithm) {
        try {
            if (ProofType.ML_DSA_44_SIGNATURE_2024.getRawValue().equals(algorithm)) {
                return walletManager.generateCompactSignatureFromHash(keyId, plainText);
            }
            return walletManager.generateCompactSignatureFromHash(keyId, BaseDigestUtil.generateHash(plainText));
        } catch (WalletException e) {
            log.error("Failed to generate compact signature: {}", e.getMessage());
    ...
    public static void generateKeyPairSafe(WalletManagerInterface walletManager, String keyId) {
        generateKeyPairSafe(walletManager, keyId, CryptoKeyPairInfo.KeyAlgorithmType.SECP256r1);
    }

    // forten - 알고리즘 지정 키 생성 (safe)
    public static void generateKeyPairSafe(WalletManagerInterface walletManager, String keyId, CryptoKeyPairInfo.KeyAlgorithmType algorithmType) {
        try {
            generateKeyPair(walletManager, keyId, algorithmType);
        } catch (OpenDidException e) {
    ...
    // forten - keyId별 알고리즘 지정 키 초기화
    public static WalletManagerInterface initializeWalletWithKeys(String walletFilePath, String password, Map<String, CryptoKeyPairInfo.KeyAlgorithmType> keyAlgorithms) {
        createFileWalletSafe(walletFilePath, password);
        WalletManagerInterface walletManager = connectFileWallet(walletFilePath, password);
        for (Map.Entry<String, CryptoKeyPairInfo.KeyAlgorithmType> entry : keyAlgorithms.entrySet()) {
            generateKeyPairSafe(walletManager, entry.getKey(), entry.getValue());
        }
        return walletManager;
    }
}