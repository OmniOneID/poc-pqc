...
public class WalletServiceImpl implements WalletService {
    ...
    public RegisterWalletResDto RequestRegisterWallet(RegisterWalletReqDto registerWalletReqDto) {
        try {
            ...
            // Check for duplicate Wallet ID (idempotent: same walletId+DID skips re-registration).
            log.debug("\t--> Checking Wallet ID Duplicate");
            String walletId = registerWalletReqDto.getAttestedDidDoc().getWalletId();
            String ownerDid = ownerDidDoc.getId();
            boolean alreadyRegistered = walletQueryService.countByWalletId(walletId) > 0;
            if (!alreadyRegistered) {
                checkWalletIdDuplicate(walletId);
                ...
                log.debug("\t--> Wallet already registered, skipping re-registration");
            }
    ...
    private byte[] extractSignatureMessage(AttestedDidDoc attestedDidDoc) {
        try {
            ...
            // PQC: ML-DSA-44는 raw bytes로 서명하므로 hash 없이 반환; EC는 SHA-256 hash 사용
            if (ProofType.ML_DSA_44_SIGNATURE_2024.equals(attestedDidDoc.getProof().getType())) {
