...
2. **Create a Release Branch**  
   If there are changes or modifications, create a branch "release/QA-VX.Y.Z" for QA validation.
    - Example: If there are bug fixes or minor improvements for V3.0.0, create a branch "release/QA-V3.0.0".

   For modules without changes, use the existing version (V3.0.0) and the already distributed JAR or library.
...
1. **Managing QA Request Branches**
    - Create a directory for the QA request version (format: W.X.Y.Z, e.g., V3.0.0.0). The directory name should be /release-VW.X.Y.Z (e.g., /release-V2.0.0.0).
...
3. **Publishing the Integrated Release**
   ...
   ## Release Note V3.0.0.0

   | Repository            | Version | Changelog                                                                              | Release |
      |-----------------------|---------|----------------------------------------------------------------------------------------| ------- |
   | did-oid4vp-sdk-server | V3.0.0  | [Changelog](https://github.com/OmniOneID/did-oid4vp-sdk-server/blob/main/CHANGELOG.md) |         |
