...
import org.omnione.did.sdk.core.keymanager.supportalgorithm.MlDsa44Manager;
...
class KeyManager<E extends BaseObject>{
    ...
    // ML-DSA-44 bio-wrapped blob layout: [IV (12 bytes) || AES-GCM ciphertext (incl. 16-byte tag)]
    private static final int BIO_WRAP_IV_LEN = 12;
    private byte[] unlockedBioPrivateKey;
    private String unlockedBioKeyId;
    public KeyManager(){}
    ...
    public void authenticatePin(String id, byte[] pin) throws WalletCoreException, UtilityException {
        ...
        switch (signkeyInfo.getAccessMethod()) {
            case WALLET_PIN:
                ...
                SignableInterface keyAlgorithmForAuth = getKeyAlgorithm(signkeyInfo.getAlgorithm());
                if (keyAlgorithmForAuth instanceof Secp256R1Manager) {
                    ((Secp256R1Manager) keyAlgorithmForAuth).checkKeyPairMatch(PINDecoded, publicKey);
                } else if (keyAlgorithmForAuth instanceof MlDsa44Manager) {
                    ((MlDsa44Manager) keyAlgorithmForAuth).checkKeyPairMatch(PINDecoded, publicKey);
                } else if (keyAlgorithmForAuth instanceof MlKem768Manager) {
                    ((MlKem768Manager) keyAlgorithmForAuth).checkKeyPairMatch(PINDecoded, publicKey);
                }
    ...
    private void generateKeyForWallet(WalletKeyGenRequest keyGenRequest) throws WalletCoreException, UtilityException {
        ...
        SignableInterface keyAlgorithm = getKeyAlgorithm(keyGenRequest.getAlgorithmType());
    ...
    /**
     * Generates a BIO-protected ML-DSA-44 key in the hybrid scheme: the ML-DSA private key is
     * generated in memory and wrapped by the provided AES-GCM {@link Cipher} (already authorized
     * via a successful BiometricPrompt bound to the Keystore wrapping key). The wrapped blob and
     * public key are persisted as a regular {@link KeyInfo}/{@link DetailKeyInfo} pair with
     * {@code accessMethod = KEYSTORE_BIOMETRY} and {@code algorithm = ML_DSA_44}.
     *
     * The caller (WalletCore) must ensure the cipher is freshly authenticated and init'd for
     * {@link Cipher#ENCRYPT_MODE}; this method reads {@code cipher.getIV()} and performs a single
     * {@code doFinal}.
     */
    public void generateBioMlDsaKey(String id, AlgorithmType.ALGORITHM_TYPE algorithmType, Cipher authenticatedEncryptCipher) throws WalletCoreException, UtilityException {
        if (id == null || id.isEmpty()) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "id");
        }
        if (algorithmType != AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_UNSUPPORTED_ALGORITHM, algorithmType.getValue());
        }
        if (authenticatedEncryptCipher == null) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "cipher");
        }
        if (storageManager.isSaved()) {
            List<KeyInfo> existingMetas = storageManager.getAllMetas();
            for (KeyInfo existing : existingMetas) {
        ...
        SignableInterface keyAlgorithm = getKeyAlgorithm(algorithmType);
        KeyGenerationInfo genKeyInfo = keyAlgorithm.generateKey();
        if (genKeyInfo.getPrivateKey() == null) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "genKeyInfo");
        }
        byte[] priKey = MultibaseUtils.decode(genKeyInfo.getPrivateKey());
        byte[] iv = authenticatedEncryptCipher.getIV();
        if (iv == null || iv.length != BIO_WRAP_IV_LEN) {
            Arrays.fill(priKey, (byte) 0x00);
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "iv");
        }
        byte[] ciphertext;
        try {
            ciphertext = authenticatedEncryptCipher.doFinal(priKey);
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            Arrays.fill(priKey, (byte) 0x00);
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_UNSUPPORTED_ALGORITHM, e.getMessage());
        }
        ...
        KeyInfo keyInfo = new KeyInfo(
                ...
                AuthType.AUTH_TYPE.BIO,
                algorithmType,
                genKeyInfo.getPublicKey(),
                KeyAccessMethod.KEY_ACCESS_METHOD.KEYSTORE_BIOMETRY
        );
        DetailKeyInfo detailKeyInfo = new DetailKeyInfo(
                id,
                MultibaseUtils.encode(MultibaseType.MULTIBASE_TYPE.BASE_58_BTC, blob)
        );
        storageManager.addItem(getUsableInnerWalletItem(keyInfo, detailKeyInfo), false);

        // Bio is considered "unlocked" immediately after registration: the biometric prompt that
        // authorized encryption counts as the active session's auth. Post-register flows (e.g. DID
        // update signing with ["pin","bio"]) can therefore sign without an additional prompt.
        clearUnlockedBioKey();
        unlockedBioPrivateKey = priKey;
        unlockedBioKeyId = id;

        Arrays.fill(ciphertext, (byte) 0x00);
        Arrays.fill(blob, (byte) 0x00);
        genKeyInfo.setPrivateKey(null);
    }

    /**
     * Returns the 12-byte IV prefix from a BIO ML-DSA key's wrapped blob. Called by WalletCore
     * before showing the BiometricPrompt so it can initialize the DECRYPT cipher properly.
     */
    public byte[] getBioWrappedIv(String id) throws WalletCoreException, UtilityException {
        if (id == null || id.isEmpty()) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "id");
        }
        List<UsableInnerWalletItem<KeyInfo, DetailKeyInfo>> items = storageManager.getItems(List.of(id));
        DetailKeyInfo detail = items.get(0).getItem();
        byte[] blob = MultibaseUtils.decode(detail.getPrivateKey());
        if (blob == null || blob.length < BIO_WRAP_IV_LEN) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_FAILED_TO_DECODE, "Data(R)");
        }
        byte[] iv = new byte[BIO_WRAP_IV_LEN];
        System.arraycopy(blob, 0, iv, 0, BIO_WRAP_IV_LEN);
        return iv;
    }

    /**
     * Decrypts the wrapped ML-DSA private key using the provided (biometric-authenticated)
     * {@link Cipher} and stashes the plaintext in a transient field. The stash is consumed by the
     * next {@link #sign(String, byte[], byte[])} call for this {@code id} and then zeroed.
     */
    public void unlockBioMlDsaKey(String id, Cipher authenticatedDecryptCipher) throws WalletCoreException, UtilityException {
        if (id == null || id.isEmpty()) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "id");
        }
        if (authenticatedDecryptCipher == null) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "cipher");
        }
        List<UsableInnerWalletItem<KeyInfo, DetailKeyInfo>> items = storageManager.getItems(List.of(id));
        DetailKeyInfo detail = items.get(0).getItem();
        ...
        byte[] ciphertext = new byte[blob.length - BIO_WRAP_IV_LEN];
        System.arraycopy(blob, BIO_WRAP_IV_LEN, ciphertext, 0, ciphertext.length);
        try {
            clearUnlockedBioKey();
            unlockedBioPrivateKey = authenticatedDecryptCipher.doFinal(ciphertext);
            unlockedBioKeyId = id;
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_UNSUPPORTED_ALGORITHM, e.getMessage());
        } finally {
    ...
    /**
     * Zeroes and forgets any plaintext BIO private key previously set by
     * {@link #unlockBioMlDsaKey(String, Cipher)}.
     */
    public void clearUnlockedBioKey() {
        if (unlockedBioPrivateKey != null) {
            Arrays.fill(unlockedBioPrivateKey, (byte) 0x00);
        }
        unlockedBioPrivateKey = null;
        unlockedBioKeyId = null;
    }
    ...
    public void deleteKeys(List<String> ids) throws WalletCoreException, UtilityException {
        ...
        for(KeyInfo meta : metas){
            if(meta.getAccessMethod().getValue() == KeyAccessMethod.KEY_ACCESS_METHOD.KEYSTORE_BIOMETRY.getValue()) {
                ...
                KeystoreManager.deleteKey(KeystoreManager.BIO_WRAPPING_KEY_ALIAS_PREFIX, meta.getId());
            }
        }
    ...
    public byte[] sign(String id, byte[] pin, byte[] digest) throws WalletCoreException, UtilityException {
        ...
        switch (signkeyInfo.getAccessMethod()) {
            case WALLET_NONE:
                ...
                SignableInterface keyGenForSign = getKeyAlgorithm(signkeyInfo.getAlgorithm());
                signature = keyGenForSign.sign(decPrivateKey, digest);
            ...
            case WALLET_PIN:
                ...
                SignableInterface keyAlgorithmForPin = getKeyAlgorithm(signkeyInfo.getAlgorithm());
                if (keyAlgorithmForPin instanceof Secp256R1Manager) {
                    ((Secp256R1Manager) keyAlgorithmForPin).checkKeyPairMatch(PINDecoded, publicKey);
                } else if (keyAlgorithmForPin instanceof MlDsa44Manager) {
                    ((MlDsa44Manager) keyAlgorithmForPin).checkKeyPairMatch(PINDecoded, publicKey);
                } else if (keyAlgorithmForPin instanceof MlKem768Manager) {
                    ((MlKem768Manager) keyAlgorithmForPin).checkKeyPairMatch(PINDecoded, publicKey);
                }
                signature = keyAlgorithmForPin.sign(PINDecoded, digest);
                Arrays.fill(pin, (byte) 0x00);
                Arrays.fill(multibaseDecoded, (byte) 0x00);
                Arrays.fill(salt, (byte) 0x00);
                Arrays.fill(symmetricKey, (byte) 0x00);
            ...
            case KEYSTORE_BIOMETRY:
                if (signkeyInfo.getAlgorithm() == AlgorithmType.ALGORITHM_TYPE.ML_DSA_44) {
                    if (unlockedBioPrivateKey == null || !id.equals(unlockedBioKeyId)) {
                        throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_INVALID_PARAMETER, "bio key not unlocked");
                    }
                    SignableInterface bioAlgo = getKeyAlgorithm(signkeyInfo.getAlgorithm());
                    if (bioAlgo instanceof MlDsa44Manager) {
                        ((MlDsa44Manager) bioAlgo).checkKeyPairMatch(unlockedBioPrivateKey, publicKey);
                    }
                    // Unlocked key persists for the session to support multi-sign flows
                    // (e.g. addProofsToDocument(["pin","bio"]) + getSignedDIDAuth back-to-back).
                    // It's cleared on next authenticateBioKey() or explicit clearBioUnlock().
                    signature = bioAlgo.sign(unlockedBioPrivateKey, digest);
                } else {
                    signature = KeystoreManager.sign(id, digest);
                }
                break;
    ...
    public boolean verify(AlgorithmType.ALGORITHM_TYPE algorithmType, byte[] publicKey, byte[] digest, byte[] signature) throws WalletCoreException {
        ...
        SignableInterface verifier = getKeyAlgorithm(algorithmType);
    ...
    private <T extends SignableInterface> T getKeyAlgorithm(AlgorithmType.ALGORITHM_TYPE algorithmType) throws WalletCoreException{
        switch (algorithmType){
            ...
            case ML_DSA_44:
                MlDsa44Manager mlDsa44Manager = new MlDsa44Manager();
                return (T) mlDsa44Manager;
            case ML_KEM_768:
                MlKem768Manager mlKem768Manager = new MlKem768Manager();
                return (T) mlKem768Manager;
            default:
                throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEY_MANAGER_UNSUPPORTED_ALGORITHM, algorithmType.getValue());
        }
    }
