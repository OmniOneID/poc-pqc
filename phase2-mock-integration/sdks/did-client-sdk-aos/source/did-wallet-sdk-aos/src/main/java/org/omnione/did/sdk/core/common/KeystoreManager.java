...
public class KeystoreManager {
    ...
    public static final String BIO_WRAPPING_KEY_ALIAS_PREFIX = "opendid_wallet_bio_wrap_";
...
//            }
    ...
    /**
     * Generates (or returns existing) AES-256-GCM AndroidKeystore key used to wrap a PQC private key
     * for a biometric-protected hybrid key. The key requires per-use user authentication via
     * BiometricPrompt.CryptoObject. It is not invalidated on biometric re-enrollment.
     *
     * @param context Android context
     * @param alias Fully-qualified Keystore alias (e.g. {@code BIO_WRAPPING_KEY_ALIAS_PREFIX + keyId})
     * @return The SecretKey usable for Cipher.init with a CryptoObject-bound BiometricPrompt
     */
    public static SecretKey generateOrGetBioWrappingKey(Context context, String alias) throws WalletCoreException {
        try {
            KeyStore keyStore = KeyStore.getInstance("AndroidKeyStore");
            keyStore.load(null);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                // Per-use authentication (timeout=0) via strong biometric
            ...
            return keyGenerator.generateKey();
                ...
                | NoSuchProviderException | InvalidAlgorithmParameterException | UnrecoverableEntryException e) {
            throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEYSTORE_MANAGER_FAILED_TO_CREATE_SECURE_KEY, e.getMessage());
        }
    }

    /**
     * Returns the bio wrapping key if present, else throws.
     */
    public static SecretKey getBioWrappingKey(String alias) throws WalletCoreException {
        try {
            if (!isExistAliasInKeyStore(alias)) {
                throw new WalletCoreException(WalletCoreErrorCode.ERR_CODE_KEYSTORE_MANAGER_CANNOT_FIND_SECURE_KEY_BY_CONDITION);
            }
