...
public class RegUser {
        ...
        return CompletableFuture.supplyAsync(() -> {
                ...
                .thenCompose(_M132_ProposeRegisterUser -> {
                    ...
                    if (org.omnione.did.sdk.wallet.walletservice.config.Config.isMlKemKeyAgreement()) {
                        return CompletableFuture.supplyAsync(() -> httpUrlConnection.send(context, Config.TAS.REQUEST_ML_KEM, "POST", M132_RequestMlKem()));
                    } else {
                        return CompletableFuture.supplyAsync(() -> httpUrlConnection.send(context, Config.TAS.REQUEST_ECDH, "POST", M132_RequestEcdh()));
                    }
                })
                .thenCompose(_M132_RequestKeyAgree -> {
                    ecdhResult = _M132_RequestKeyAgree;
                    if (org.omnione.did.sdk.wallet.walletservice.config.Config.isMlKemKeyAgreement()) {
                        // Option 1: server returned ciphertext → decap with ephemeral sk to recover sharedSecret
                        try {
                            P132ResponseVo respVo = MessageUtil.deserialize(_M132_RequestKeyAgree, P132ResponseVo.class);
                            ...
                            mlKemEphemeralSk = null; // best-effort reference drop
                        } catch (WalletCoreException | UtilityException e) {
                            throw new CompletionException(e);
                        }
                    }
                    // ECDH: ecdhResult에서 서버 공개키를 꺼내 후속 처리 (다음 단계)
                ...
                .thenCompose(_M132_RequestCreateToken -> {
                    try {
                        if (org.omnione.did.sdk.wallet.walletservice.config.Config.isMlKemKeyAgreement()) {
                            serverToken = TokenUtil.createServerTokenMlKem(_M132_RequestCreateToken, ecdhResult, clientNonce, mlKemSharedSecret);
                        } else {
    ...
    private String M132_RequestMlKem(){
        // Option 1 (Wallet-as-receiver):
        //  - generate ephemeral ML-KEM-768 keypair
        //  - send own public key; server will encap and return ciphertext in accMlKem
        //  - NO pre-lookup of TAS DID Doc (server long-term KEM key is no longer needed)
        P132RequestVo requestVo = new P132RequestVo(CaUtil.createMessageId(context), txId);
        ReqMlKem reqMlKem = new ReqMlKem();
        try {
            WalletApi walletApi = WalletApi.getInstance(context);
            String did = walletApi.getDIDDocument(Constants.DID_TYPE_DEVICE).getId();

            MlKem768Manager kemManager = new MlKem768Manager();
            org.omnione.did.sdk.core.keymanager.datamodel.KeyGenerationInfo ephPair = kemManager.generateKey();
            // Stash raw PKCS8 private key bytes for post-response decap.
            mlKemEphemeralSk = MultibaseUtils.decode(ephPair.getPrivateKey());
            String ephPublicKeyMultibase = ephPair.getPublicKey(); // multibase-encoded X.509 SPKI
            ...
            reqMlKem = (ReqMlKem) walletApi.addProofsToDocument(reqMlKem, List.of("keyagree"), did, Constants.DID_TYPE_DEVICE, null, false);

            // sharedSecret will be recovered in the RES handler via Decap(ephemeralSk, serverCt)
        } catch (Exception e) {
