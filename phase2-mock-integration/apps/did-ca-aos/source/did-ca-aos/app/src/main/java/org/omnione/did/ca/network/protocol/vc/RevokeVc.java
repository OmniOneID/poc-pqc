...
public class RevokeVc {
        ...
        return CompletableFuture.supplyAsync(() -> httpUrlConnection.send(context, Config.TAS.BASE_URL + api1, "POST", M220_ProposeRevokeVc(vcId)))
                .thenCompose(_M220_ProposeRevokeVc -> {
                    ...
                    if (org.omnione.did.sdk.wallet.walletservice.config.Config.isMlKemKeyAgreement()) {
                        final String proposeResult = _M220_ProposeRevokeVc;
                        return CompletableFuture.supplyAsync(() -> httpUrlConnection.send(context, Config.TAS.BASE_URL + api2, "POST", M220_RequestMlKem(proposeResult)));
                    } else {
                        final String proposeResult = _M220_ProposeRevokeVc;
                        return CompletableFuture.supplyAsync(() -> httpUrlConnection.send(context, Config.TAS.BASE_URL + api2, "POST", M220_RequestEcdh(proposeResult)));
                    }
                })
                .thenCompose(_M220_RequestEcdh -> {
                    ...
                    if (org.omnione.did.sdk.wallet.walletservice.config.Config.isMlKemKeyAgreement()) {
                        ...
                        try {
                            P220ResponseVo respVo = MessageUtil.deserialize(_M220_RequestEcdh, P220ResponseVo.class);
                ...
                .thenApply(_M220_RequestCreateToken -> {
                    try {
                        if (org.omnione.did.sdk.wallet.walletservice.config.Config.isMlKemKeyAgreement()) {
                            serverToken = TokenUtil.createServerTokenMlKem(_M220_RequestCreateToken, ecdhResult, clientNonce, mlKemSharedSecret);
                        } else {
    ...
    private String M220_RequestMlKem(String result){
