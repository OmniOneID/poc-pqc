...
public class TokenUtil {
    ...
    /**
     * ML-KEM용 createServerToken.
     * sharedSecret은 이미 encapsulate에서 획득했으므로,
     * 서버 응답(AccMlKem)의 serverNonce와 병합하여 세션 키를 유도합니다.
     */
    public static String createServerTokenMlKem(String std, String mlKemResult, byte[] clientNonce, byte[] mlKemSharedSecret) throws UtilityException {
        String iv = MessageUtil.deserialize(std, P210ResponseVo.class).getIv();
        String encStd = MessageUtil.deserialize(std, P210ResponseVo.class).getEncStd();
        // 서버 응답은 {"txId":"...", "accMlKem":{...}} 구조이므로 accMlKem 필드를 추출하여 파싱
        JsonObject root = JsonParser.parseString(mlKemResult).getAsJsonObject();
        AccMlKem accMlKem = new com.google.gson.Gson().fromJson(root.get("accMlKem"), AccMlKem.class);
        ...
        byte[] serverNonce = MultibaseUtils.decode(accMlKem.getServerNonce());
        byte[] mergedNonce = mergeNonce(clientNonce, serverNonce);
        byte[] sessKey = mergeSharedSecretAndNonce(mlKemSharedSecret, mergedNonce, SymmetricCipherType.SYMMETRIC_CIPHER_TYPE.AES256CBC);

        byte[] plain = CryptoUtils.decrypt(MultibaseUtils.decode(encStd), info, sessKey, MultibaseUtils.decode(iv));
        String serverToken = MultibaseUtils.encode(MultibaseType.MULTIBASE_TYPE.BASE_58_BTC, DigestUtils.getDigest(new String(plain).getBytes(), DigestEnum.DIGEST_ENUM.SHA_256));
        CaLog.d("serverToken by CA (ML-KEM) : " + serverToken);
        return serverToken;
    }
