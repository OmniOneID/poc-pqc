...
To support the PQC algorithms used in Phase 2 (**ML-DSA-44** for signatures, **ML-KEM-768** for key agreement), the CA app pulls in **Bouncy Castle 1.80** alongside the legacy SpongyCastle 1.54 stack:

```groovy
// app/build.gradle
configurations.all {
    // Avoid pulling an older BC transitively
    exclude group: 'org.bouncycastle', module: 'bcprov-jdk15to18'
}

dependencies {
    // Existing classical-crypto stack (kept for ECDSA / Secp256r1 paths)
    implementation 'com.madgag.spongycastle:core:1.54.0.0'
    implementation 'com.madgag.spongycastle:prov:1.54.0.0'
    implementation 'com.madgag.spongycastle:pkix:1.54.0.0'
    implementation 'com.madgag.spongycastle:pg:1.54.0.0'
    ...
    // bitcoinj brings its own BC; exclude to avoid provider duplication
    implementation('org.bitcoinj:bitcoinj-core:0.15.7') {
...
- **SpongyCastle 1.54 stays** — existing key managers (`Secp256R1Manager`, `Secp256K1Manager`) and ECDSA flows still use the `org.spongycastle.*` package. Removing it would require rewriting every classical path.
- **BC 1.80 is required for PQC** — FIPS 203 (ML-KEM) and FIPS 204 (ML-DSA) only land in Bouncy Castle 1.78+. The PQC managers in the SDK (`MlDsa44Manager`, `MlKem768Manager`) statically swap Android's bundled `"BC"` provider for `BouncyCastleProvider` from 1.80 at class-init time so JCE calls (`KeyPairGenerator.getInstance("ML-DSA-44", "BC")` etc.) resolve to the new provider. SpongyCastle is **not** removed during this swap.
- **The two excludes** (`bcprov-jdk15to18` globally and `bitcoinj-core`'s transitive BC) prevent older `org.bouncycastle.*` jars from entering the classpath and shadowing 1.80 at runtime.

If you target a server using classical Secp256r1 / ECDH only, the PQC dependency is harmless — the PQC managers are only used when `pqc.signature-algorithm` / `pqc.key-agreement-algorithm` on the server are set to `MlDsa44` / `MlKem768`.
